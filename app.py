...
class SEOManager:
    def __init__(self, df, user_exclude_list):
        ...
        # NLU 분리 기준 키워드 (길이가 긴 것부터 확인하여 오차 방지)
        self.sub_splits = sorted(['자판기', '우유', '분유', '가루', '분말', '전지', '탈지', '스틱', '업소용', '대용량', '식자재', '제과', '제빵', '베이킹'], key=len, reverse=True)

    def split_base_terms(self, text):
        """복합 명사를 여러 개의 NLU 단위로 일괄 분리하는 정교한 로직"""
        if pd.isna(text) or text == '-': return []
        
        # 1차: 특수문자 제거 및 공백 정규화
        text = re.sub(r'[^가-힣a-zA-Z0-9\s]', ' ', str(text))
        
        # 2차: 정규표현식을 이용해 sub_splits에 포함된 모든 단어를 기준으로 쪼개기
        ...
        for part in raw_splits:
            part = part.strip()
            # 불용어, 숫자, 브랜드명 필터링
            if not part or part in self.exclude_brands or any(char.isdigit() for char in part):
                continue
            if len(part) > 1:
                final_tags_clean = re.sub(r'\s+', ' ', part)
                final_terms.extend(final_tags_clean.split())
            elif part in self.sub_splits: # 1글자라 하더라도 주요 키워드인 경우 포함 (예: '맛')
                final_terms.append(part)
        
        return [t for t in final_terms if t not in self.exclude_brands]
    ...
    def run_analysis(self, conversion_input, add_input, total_target_count):
        # 입력값 분리 (순서 유지를 위해 입력된 순서대로 split)
        ...
        candidates = []
        for t, c in tag_freq:
            if not any(char.isdigit() for char in t) and not any(word == t for word in current_title_words):
...
add_input = st.sidebar.text_input("추가할 키워드", placeholder="예: 무료배송")
exclude_input = st.sidebar.text_input("제외 키워드")
...
if uploaded_file:
    ...
    manager = SEOManager(df, [w.strip() for w in exclude_input.split() if w.strip()])
    ...
    with col1:
        ...
        # 고정 키워드는 입력된 순서대로, 자동 키워드는 가독성 순서대로 배치
    ...
    st.markdown("---")
    st.header("⚙️ 2. 속성 및 🔍 3. 태그")
    t1, t2 = st.columns(2)
    with t1:
        st.subheader("필터 속성")
        for s, _ in specs: st.button(s, key=f"at_{s}", use_container_width=True)
    with t2:
        st.subheader("확장 태그 10선")
