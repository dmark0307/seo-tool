...
# 1. 페이지 설정 및 디자인 최적화 (사이드바 스크롤 방지 CSS 유지)
...
class SEOManager:
    ...
    def split_base_terms(self, text, is_manual=False):
        ...
        for word in raw_words:
            ...
            for p in parts:
                ...
                # 수동 입력(고정 배치 등)은 1글자나 특수 조건에서도 최대한 보존
    ...
    def run_analysis(self, stats_keywords, conversion_input, add_input, total_target_count):
        ...
        # 2. 고정 키워드 리스트 생성 
        # 배치 순서: [통계 반영] → [고정 배치] → [구매전환 추가]
        fixed_keywords = []
        # (1) 통계 반영 키워드 우선 배치
        for k in stats_keywords:
            if k not in fixed_keywords: fixed_keywords.append(k)
        # (2) 고정 배치 키워드(add_input) 두 번째 배치
        for k in manual_add:
            if k not in fixed_keywords: fixed_keywords.append(k)
        # (3) 구매전환 추가 키워드(conversion_input) 세 번째 배치
